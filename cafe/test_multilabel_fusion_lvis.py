...
class Model(todd.Module):
    ...
    def forward(self, batch: Batch) -> Dict[str, Any]:
        ...
        bboxes, labels = multiclass_nms(
            batch.bboxes[:2], ensemble_score[:2, self._num_base_classes:],
            **self._nms_cfg,
        )
        labels += self._num_base_classes
        return {
...
if __name__ == '__main__':
    ...
    config.data.test.ann_file = 'data/lvis_v1/annotations/lvis_v1_val.json.LVIS.337'
    config = todd.Config(
        val=dict(
            dataloader=dict(
                batch_size=1,
