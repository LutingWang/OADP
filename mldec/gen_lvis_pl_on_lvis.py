...
class Runner(BaseRunner):
    ...
    def _after_run(self, *args, memo: Dict[str, Any], **kwargs) -> float:
        ...
        lvis_anno = lvisGt.dataset['annotations']
        new_categories = []
        name2cat =dict()
        id2name = dict()
        ...
        lvis_names = getattr(datasets, 'LVIS')

        base_name = getattr(datasets, 'LVIS_866')
        new_pl = 0
        new_pls = []
        new_id = 0

        # for anno in lvis_anno:
        #     class_name = id2name[anno['category_id']]
        #     if class_name not in base_name:
        #         continue
        #     else:
        #         anno['category_id'] = lvis_names.index(class_name)
        #         anno['id'] = (new_id+1)
        #         new_id += 1
        #         new_pls.append(anno)
        ...
        for i,data in enumerate(new_annotations):
            ...
            if class_name in base_name:
                # import ipdb;ipdb.set_trace()
                image_not_exhaus = lvisGt.imgs[data['image_id']]['not_exhaustive_category_ids']
                if data['category_id'] not in image_not_exhaus:
                    continue
                base_pl += 1
            else:
                new_pl += 1
        ...
        self._logger.info("Generate novel pl: "+str(new_pl))
        self._logger.info("Generate base pl: "+str(base_pl))
