...
class LoadCLIPFeatures:
    ...
    def __init__(
        ...
        self._globals: Mapping[str, torch.Tensor] | None = (
            None if globals is None else ALR.build(globals, default)
        )
        self._blocks: Mapping[str, dict[str, torch.Tensor]] | None = (
            None if globals is None else ALR.build(blocks, default)
        )
        self._objects: Mapping[str, dict[str, torch.Tensor]] | None = (
            None if globals is None else ALR.build(objects, default)
        )
    ...
    def __call__(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._globals is not None:
            image = self._globals[key]
            results['clip_globals'] = image.squeeze(0)
