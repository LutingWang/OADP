...
class Debug(todd.base.BaseDebug):
    ...
    def init_cpu(self, **kwargs) -> None:
        ...
        mmcv.cnn.NORM_LAYERS.register_module(
            name='SyncBN', force=True, module=nn.BatchNorm2d
        )

    def init(
        self, *, config: Optional[todd.base.Config] = None, **kwargs
    ) -> None:
        super().init(**kwargs)
        if config is None:
            return
        if self.TRAIN_WITH_VAL_DATASET:
            val_dataset = {
                ...
                for k in config.train.dataloader.dataset.keys()
                & config.val.dataloader.dataset.keys()
            }
