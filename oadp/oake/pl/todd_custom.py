...
class BaseRunner(ABC):
    ...
    def load_checkpoint(
        ...
        *args,
        epoch,
        config: Optional[todd.Config] = None,
        **kwargs
    ) -> None:
        model = self._model
        ...
        if config is None:
            config = todd.getattr_recur(
                self._config, '.checkpoint.load_', dict()
            )
        todd.load_checkpoint(
            model,
            self._work_dir / f'epoch_{epoch}.pth',
            **config,
        )
    ...
    def _before_run_iter(
        self, *args, i: int, batch, memo: Dict[str, Any], **kwargs
    ) -> None:
        pass
        ...
        for i, batch in enumerate(self._dataloader, 1):
            ...
            try:
                ...
                *args,
                ...
                log=(
                    i
                    % todd.getattr_recur(self._config, '.logger.interval',
                                         1) == 0
                ),
                **kwargs,
            )
...
class TrainerMixin(BaseRunner):
    ...
    def save_checkpoint(
        ...
        **kwargs
        ...
        if config is None:
            config = todd.getattr_recur(
                self._config, '.checkpoint.save', dict()
            )
    ...
    def _before_train_epoch(
        self, *args, epoch: int, memo: Dict[str, Any], **kwargs
    ) -> None:
        if isinstance(self._train_sampler, data.DistributedSampler):
            self._train_sampler.set_epoch(epoch)
        self._model.train()

    def _before_train_iter(
        self, *args, epoch: int, i: int, batch, memo: Dict[str, Any], **kwargs
    ) -> None:
        pass
    ...
    def train(self, *args, **kwargs):
        ...
        for epoch in range(self._epoch + 1, self._config.train.epoch):
            ...
            for i, batch in enumerate(self._train_dataloader, 1):
                self._before_train_iter(
                    *args, epoch=epoch, i=i, batch=batch, memo=memo, **kwargs
                )
                try:
                    self._train_iter(
                        *args,
                    ...
                    *args,
                    ...
                    log=(
                        i % todd.
                        getattr_recur(self._config, '.logger.interval', 1) == 0
                    ),
                    **kwargs,
                )
                if end:
                    break
            end = self._after_train_epoch(
                *args, epoch=epoch, memo=memo, **kwargs
            )
            if end:
                break
...
class HierGKDLoss(todd.losses.functional.CrossEntropyLoss):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights: torch.Tensor = torch.einsum(
            'a c, b c n -> a b n', targets, fused_preds
        )
        attn_weights = attn_weights.softmax(-1)
        values = torch.einsum(
            'a b n, b c n -> a c b', attn_weights, fused_preds
        )
        values = F.normalize(values)
        logits: torch.Tensor = torch.einsum(
            'a c, a c b -> a b', targets, values
        )
