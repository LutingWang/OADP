...
from ...base.globals_ import coco, lvis
...
Batch = namedtuple(
    ...
    [
        ...
        'image_ids', 'class_embeddings', 'scaler', 'bias', 'mask'
    ],
)
...
class CocoClassification(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(
        ...
        return proposal_embeddings[inds], proposal_objectness[
            inds], proposal_bboxes[inds], image_id
...
class Model(todd.base.Module):
    ...
    def forward(self, batch: Batch) -> torch.Tensor:
        ...
        clip_logit_v, clip_logit_i = torch.topk(
        ...
        if self.score_fusion_cfg['_name'] == 'add':
            ...
            final_logit_k = (
                ...
                batch.proposal_objectness[..., None] **
                self.score_fusion_cfg['obj_score_ratio']
            )
        else:
...
class Runner(BaseRunner):
    ...
    def _after_run_iter(
        ...
        **kwargs
        ...
        if log and todd.get_rank() == 0:
            self._logger.info(f'Val Step [{i}/{len(self._dataloader)}]')
    ...
    def _after_run(self, *args, memo: Dict[str, Any], **kwargs) -> float:
        ...
        if self._config.model.dis:
            ...
            results: Iterator[Tuple[torch.Tensor, ...]] = zip(*memo['results'])
            results = tuple(
                # all_gather must exec for all ranks ,
                # may result in cuda out of memory
        ...
        json_path = os.path.join(self._work_dir, self._config.json_name)
...
if __name__ == '__main__':
    ...
    config.val.dataloader.sample = not args.hotwater
    config.model.dis = not args.hotwater
