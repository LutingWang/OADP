...
class LVISVis:
    ...
    def get_synset(self, idx):
        synset = self.lvis_gt.load_cats(ids=[idx])[0]["name"]
        text = synset
        return text
...
ratio = 100
lvisVis = LVISVis(lvis,lvis_dt = None,img_dir=img_dir)
for i in range(len(img_ids)//ratio):
    img_id = img_ids[i*ratio]
    if len(lvis.get_ann_ids([img_id]))== 0:
        continue 
    res = lvisVis.vis_img(img_id,show_boxes=True,show_classes=True,show_segms=False)
    res.savefig(save_fig_dir+str(img_id)+'.jpg')
