...
def check_for_positive_overflow(gt_bboxes, gt_labels, text, tokenizer, max_tokens):
    ...
    return gt_bboxes[keep_box_index], np.array(keep_gt_labels, dtype=np.long), length
...
def generate_senetence_given_labels(positive_label_list, negative_label_list, text):
    ...
    return label_to_positions, pheso_caption, label_remap_dict, label_list


@TRANSFORMS.register_module()
class RandomSamplingNegPosPseudoLabel(BaseTransform):
    ...
    def od_aug(self, results):
        ...
        for negative_label in negative_label_list:
            label_text = text[str(negative_label)] + ". "
        ...
        label_to_positions, pheso_caption, label_remap_dict, label_list = (
